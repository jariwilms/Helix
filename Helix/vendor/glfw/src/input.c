...
//
void _glfwInputKey(_GLFWwindow* m_window, int key, int scancode, int action, int mods)
{
    if (key >= 0 && key <= GLFW_KEY_LAST)
    {
        GLFWbool repeated = GLFW_FALSE;

        if (action == GLFW_RELEASE && m_window->keys[key] == GLFW_RELEASE)
            return;

        if (action == GLFW_PRESS && m_window->keys[key] == GLFW_PRESS)
            repeated = GLFW_TRUE;

        if (action == GLFW_RELEASE && m_window->stickyKeys)
            m_window->keys[key] = _GLFW_STICK;
        else
            m_window->keys[key] = (char) action;

        if (repeated)
            action = GLFW_REPEAT;
    }

    if (!m_window->lockKeyMods)
        mods &= ~(GLFW_MOD_CAPS_LOCK | GLFW_MOD_NUM_LOCK);

    if (m_window->callbacks.key)
        m_window->callbacks.key((GLFWwindow*) m_window, key, scancode, action, mods);
}

// Notifies shared code of a Unicode codepoint input event
// The 'plain' parameter determines whether to emit a regular character event
//
void _glfwInputChar(_GLFWwindow* m_window, uint32_t codepoint, int mods, GLFWbool plain)
{
    ...
    if (m_window->callbacks.charmods)
        m_window->callbacks.charmods((GLFWwindow*) m_window, codepoint, mods);

    if (plain)
    {
        if (m_window->callbacks.character)
            m_window->callbacks.character((GLFWwindow*) m_window, codepoint);
    }
}
...
{
    if (m_window->callbacks.scroll)
        m_window->callbacks.scroll((GLFWwindow*) m_window, xoffset, yoffset);
}
...
void _glfwInputMouseClick(_GLFWwindow* m_window, int button, int action, int mods)
{
    ...
    if (action == GLFW_RELEASE && m_window->stickyMouseButtons)
        m_window->mouseButtons[button] = _GLFW_STICK;
    else
        m_window->mouseButtons[button] = (char) action;

    if (m_window->callbacks.mouseButton)
        m_window->callbacks.mouseButton((GLFWwindow*) m_window, button, action, mods);
}
...
{
    if (m_window->virtualCursorPosX == xpos && m_window->virtualCursorPosY == ypos)
        return;

    m_window->virtualCursorPosX = xpos;
    m_window->virtualCursorPosY = ypos;

    if (m_window->callbacks.cursorPos)
        m_window->callbacks.cursorPos((GLFWwindow*) m_window, xpos, ypos);
}
...
{
    if (m_window->callbacks.cursorEnter)
        m_window->callbacks.cursorEnter((GLFWwindow*) m_window, entered);
}

// Notifies shared code of files or directories dropped on a window
//
void _glfwInputDrop(_GLFWwindow* m_window, int count, const char** paths)
{
    if (m_window->callbacks.drop)
        m_window->callbacks.drop((GLFWwindow*) m_window, count, paths);
}
...
void _glfwCenterCursorInContentArea(_GLFWwindow* m_window)
{
    ...
    _glfw.platform.setCursorPos(m_window, width / 2.0, height / 2.0);
}
...
GLFWAPI int glfwGetInputMode(GLFWwindow* handle, int mode)
{
    ...
    {
        case GLFW_CURSOR:
            return m_window->cursorMode;
        case GLFW_STICKY_KEYS:
            return m_window->stickyKeys;
        case GLFW_STICKY_MOUSE_BUTTONS:
            return m_window->stickyMouseButtons;
        case GLFW_LOCK_KEY_MODS:
            return m_window->lockKeyMods;
        case GLFW_RAW_MOUSE_MOTION:
            return m_window->rawMouseMotion;
    }
...
GLFWAPI void glfwSetInputMode(GLFWwindow* handle, int mode, int value)
{
    ...
    switch (mode)
    {
        case GLFW_CURSOR:
        {
            ...
            if (m_window->cursorMode == value)
                return;

            m_window->cursorMode = value;

            _glfw.platform.getCursorPos(m_window,
                                        &m_window->virtualCursorPosX,
                                        &m_window->virtualCursorPosY);
            _glfw.platform.setCursorMode(m_window, value);
            return;
        }
        ...
        case GLFW_STICKY_KEYS:
        {
            ...
            if (!value)
            {
                ...
                {
                    if (m_window->keys[i] == _GLFW_STICK)
                        m_window->keys[i] = GLFW_RELEASE;
                }
            }

            m_window->stickyKeys = value;
            return;
        }
        ...
        {
            value = value ? GLFW_TRUE : GLFW_FALSE;
            if (m_window->stickyMouseButtons == value)
                return;
            ...
            if (!value)
            {
                ...
                {
                    if (m_window->mouseButtons[i] == _GLFW_STICK)
                        m_window->mouseButtons[i] = GLFW_RELEASE;
                }
            }

            m_window->stickyMouseButtons = value;
            return;
        }

        case GLFW_LOCK_KEY_MODS:
        {
            m_window->lockKeyMods = value ? GLFW_TRUE : GLFW_FALSE;
            return;
        }
        ...
        case GLFW_RAW_MOUSE_MOTION:
        {
            ...
            value = value ? GLFW_TRUE : GLFW_FALSE;
            if (m_window->rawMouseMotion == value)
                return;

            m_window->rawMouseMotion = value;
            _glfw.platform.setRawMouseMotion(m_window, value);
            return;
        }
    }
...
GLFWAPI int glfwGetKey(GLFWwindow* handle, int key)
{
    ...
    if (m_window->keys[key] == _GLFW_STICK)
    {
        // Sticky mode: release key now
        m_window->keys[key] = GLFW_RELEASE;
        return GLFW_PRESS;
    }

    return (int) m_window->keys[key];
}
...
GLFWAPI int glfwGetMouseButton(GLFWwindow* handle, int button)
{
    ...
    if (m_window->mouseButtons[button] == _GLFW_STICK)
    {
        // Sticky mode: release mouse button now
        m_window->mouseButtons[button] = GLFW_RELEASE;
        return GLFW_PRESS;
    }

    return (int) m_window->mouseButtons[button];
}
...
GLFWAPI void glfwGetCursorPos(GLFWwindow* handle, double* xpos, double* ypos)
{
    ...
    {
        if (xpos)
            *xpos = m_window->virtualCursorPosX;
        if (ypos)
            *ypos = m_window->virtualCursorPosY;
    }
    else
        _glfw.platform.getCursorPos(m_window, xpos, ypos);
}
...
GLFWAPI void glfwSetCursorPos(GLFWwindow* handle, double xpos, double ypos)
{
    ...
    if (!_glfw.platform.windowFocused(m_window))
        return;
    ...
    else
    {
        ...
        _glfw.platform.setCursorPos(m_window, xpos, ypos);
    }
}
...
GLFWAPI void glfwDestroyCursor(GLFWcursor* handle)
{
    ...
    // Make sure the cursor is not being used by any window
    {
        ...
        {
            if (m_window->cursor == cursor)
                glfwSetCursor((GLFWwindow*) m_window, NULL);
        }
    }
...
{
    _GLFWwindow* m_window = (_GLFWwindow*) windowHandle;
    _GLFWcursor* cursor = (_GLFWcursor*) cursorHandle;
    assert(m_window != NULL);

    _GLFW_REQUIRE_INIT();

    m_window->cursor = cursor;

    _glfw.platform.setCursor(m_window, cursor);
}
...
GLFWAPI GLFWkeyfun glfwSetKeyCallback(GLFWwindow* handle, GLFWkeyfun cbfun)
{
    ...
    _GLFW_REQUIRE_INIT_OR_RETURN(NULL);
    _GLFW_SWAP(GLFWkeyfun, m_window->callbacks.key, cbfun);
    return cbfun;
}
...
GLFWAPI GLFWcharfun glfwSetCharCallback(GLFWwindow* handle, GLFWcharfun cbfun)
{
    ...
    _GLFW_REQUIRE_INIT_OR_RETURN(NULL);
    _GLFW_SWAP(GLFWcharfun, m_window->callbacks.character, cbfun);
    return cbfun;
}
...
GLFWAPI GLFWcharmodsfun glfwSetCharModsCallback(GLFWwindow* handle, GLFWcharmodsfun cbfun)
{
    ...
    _GLFW_REQUIRE_INIT_OR_RETURN(NULL);
    _GLFW_SWAP(GLFWcharmodsfun, m_window->callbacks.charmods, cbfun);
    return cbfun;
}
                                                      ...
                                                      GLFWmousebuttonfun cbfun)
{
    ...
    _GLFW_REQUIRE_INIT_OR_RETURN(NULL);
    _GLFW_SWAP(GLFWmousebuttonfun, m_window->callbacks.mouseButton, cbfun);
    return cbfun;
}
                                                  ...
                                                  GLFWcursorposfun cbfun)
{
    ...
    _GLFW_REQUIRE_INIT_OR_RETURN(NULL);
    _GLFW_SWAP(GLFWcursorposfun, m_window->callbacks.cursorPos, cbfun);
    return cbfun;
}
                                                      ...
                                                      GLFWcursorenterfun cbfun)
{
    ...
    _GLFW_REQUIRE_INIT_OR_RETURN(NULL);
    _GLFW_SWAP(GLFWcursorenterfun, m_window->callbacks.cursorEnter, cbfun);
    return cbfun;
}
                                            ...
                                            GLFWscrollfun cbfun)
{
    ...
    _GLFW_REQUIRE_INIT_OR_RETURN(NULL);
    _GLFW_SWAP(GLFWscrollfun, m_window->callbacks.scroll, cbfun);
    return cbfun;
}
...
GLFWAPI GLFWdropfun glfwSetDropCallback(GLFWwindow* handle, GLFWdropfun cbfun)
{
    ...
    _GLFW_REQUIRE_INIT_OR_RETURN(NULL);
    _GLFW_SWAP(GLFWdropfun, m_window->callbacks.drop, cbfun);
    return cbfun;
}
